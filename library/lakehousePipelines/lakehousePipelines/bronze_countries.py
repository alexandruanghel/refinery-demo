...
def landing_to_bronze(spark: SparkSession, source_base_path: str, pipeline_base_path: str, pipeline_db: str) -> None:
    ...
    #
    landing_countries = f"{source_base_path.rstrip('/')}/country_code"

    bronze_tables_base_path = f"{pipeline_base_path.rstrip('/')}/bronze"
    bronze_countries_path = f"{bronze_tables_base_path}/country_code.delta"
    bronze_countries_checkpoint = f"{bronze_tables_base_path}/country_code.checkpoint"
    ...
    # Checks
    #
    print(landing_countries)
    spark.sql(f"SELECT * FROM csv.`{landing_countries}` LIMIT 5").show()

    #
    # Landing to Bronze stream
    #
    # read the data stream and add the partition column
    stream_csv_to_delta(spark, landing_countries, bronze_countries_table, bronze_countries_path, bronze_countries_checkpoint, delimiter=",")
