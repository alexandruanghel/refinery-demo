...
def landing_to_bronze(spark: SparkSession, source_base_path: str, pipeline_base_path: str, pipeline_db: str) -> None:
    ...
    #
    landing_customers = f"{source_base_path.rstrip('/')}/customers"

    bronze_tables_base_path = f"{pipeline_base_path.rstrip('/')}/bronze"
    bronze_customers_path = f"{bronze_tables_base_path}/customers.delta"
    bronze_customers_checkpoint = f"{bronze_tables_base_path}/customers.checkpoint"

    bronze_customers_table = f"{pipeline_db}.bronze_customers"
    ...
    # Checks
    #
    print(landing_customers)
    spark.sql(f"SELECT * FROM csv.`{landing_customers}` LIMIT 5").show()

    #
    # Landing to Bronze stream
    #
    # read the data stream and add the partition column
    stream_csv_to_delta(spark, landing_customers, bronze_customers_table, bronze_customers_path, bronze_customers_checkpoint, delimiter=",")


    #
    # Show results
    #
    spark.sql(f"SELECT * FROM {bronze_customers_table}").limit(5).show()
