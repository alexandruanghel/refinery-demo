...
def bronze_to_silver(spark: SparkSession, pipeline_base_path: str, pipeline_db: str) -> None:
    ...
    silver_customers_path = f"{silver_tables_base_path}/customers.delta"
    silver_customers_checkpoint = f"{silver_tables_base_path}/customers.checkpoint"

    silver_customers_table = f"{pipeline_db}.silver_customers"
    ...
    stream_delta_to_delta(spark, bronze_customers_table, silver_customers_table, silver_customers_path, silver_customers_checkpoint)

    #
    # Show results
    #
    spark.sql(f"SELECT * FROM {silver_customers_table}").limit(5).show()
