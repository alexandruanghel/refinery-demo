...
def stream_kafka_to_delta(spark: SparkSession, raw_path: str, target_table: str, target_path: str, checkpoint_path: str, delimiter: str = ",", schema: T.StructType() = None, max_files_per_trigger: int = 1) -> None:
    if not schema:
        df = (spark
              .read
              .format("json")
    ...
    #init_delta_table(spark, schema, target_path)
    ...
    query_name = f"TransactionsToDelta-{target_table}"
...
def landing_to_bronze(spark: SparkSession, source_base_path: str, pipeline_base_path: str, pipeline_db: str) -> None:
    ...
    #
    landing_transactions = f"{source_base_path.rstrip('/')}/transactions"

    bronze_tables_base_path = f"{pipeline_base_path.rstrip('/')}/bronze"
    bronze_transactions_path = f"{bronze_tables_base_path}/transactions.delta"
    bronze_transactions_checkpoint = f"{bronze_tables_base_path}/transactions.checkpoint"
    ...
    # Checks
    #
    print(landing_transactions)
    spark.sql(f"SELECT * FROM json.`{landing_transactions}` LIMIT 5").show()

    #
    # Landing to Bronze stream
    #
    # read the data stream and add the partition column
    stream_kafka_to_delta(spark, landing_transactions, bronze_transactions_table, bronze_transactions_path, bronze_transactions_checkpoint)
